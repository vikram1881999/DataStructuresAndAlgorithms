...
public class RotateMatrix {
    ...
    public static void main( String[] args ) {
        ...
        System.out.println(Arrays.deepToString(rotateMatrix.rotateUsingSwap(mat1)));
    }
    ...
    public int[][] rotateUsingSwap( int[][] arr ) {
        int n = arr.length;
        for (int i = 0; i < n / 2; i++) {
            for (int j = 0; j < n / 2; j++) {
                int temp = arr[i][j];
                ...
                arr[i][j] = arr[j][n - 1 - i];
                arr[j][n - 1 - i] = arr[n - 1 - i][n - 1 - j - i];
                arr[n - 1 - i][n - 1 - j - i] = arr[n - 1 - j - i][i];
                arr[n - 1 - j - i][n - 1 - i] = temp;
